...
## Current Fingerprinting Weakness
The `makeFingerprint()` function in `src/lib/dedup.ts` uses a collision-prone approach: first 30 chars + total length (e.g., `"the quick brown fox jumps over-43"`).
...
**Better alternatives:**
- Cosine similarity on sentence embeddings (infrastructure already exists via `EmbeddingsProvider`)
- Proper content hash (MD5/SHA) of normalized text
