...
- Display guard uses `displayGuardMapRef` with normalized text (30s in `/write`, 120s in `/demo`) to prevent duplicate visible prods. Enqueue guard uses `enqueueGuardMapRef` with fingerprints (15s in `/write`, 60s in `/demo`) to prevent duplicate queue entries. These guards operate at different pipeline stages for layered deduplication.
