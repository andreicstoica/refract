...
| `useSentenceTracker` | Sentence tokenization (punctuation + newline boundaries), measurement caching (`measureSentencePositions`) | `src/features/writing/hooks/useSentenceTracker.ts` + `src/lib/sentences.ts` |
...
- `useProdQueueManager` enforces the cadence guardrails: enqueue/display guard maps, queue rate limiting, request cancellation, topic-version staleness checks, keyword forwarding, and the cached sentence list that backs embeddings and telemetry. Instead of single-flight processing, the manager backfills as many slots as the current parallel cap allows (2 prod / 3 demo) to shrink worst-case wait time.
...
- Sentence IDs now come straight from `splitIntoSentences` and follow the reverted prefix fingerprint: `sentence-${startIndex}-${normalizedPrefix}` where the prefix is the first 20 normalized characters stripped to `[\w]`. We keep the start index component so IDs stay stable while editing earlier lines, and the prefix component ensures punctuation-free sentences still rotate once writers append new words.
- Both enqueue and display guards operate on those sentence IDs. `displayGuardMapRef` (~10 s `/write`, 30 s `/demo`) suppresses duplicate visible prods, and `enqueueGuardMapRef` (~15 s `/write`, 60 s `/demo`) blocks queue spam. Because IDs are minted upstream in `useSentenceTracker`, the queue manager no longer re-hydrates sentences before dedupe—the guard TTLs expire naturally once `useSentenceTracker` emits a new ID.
...
2. **Trigger Heuristics**
	...
	3. **Queue Gating**
		- `enqueueSentence` runs layered suppression: `shouldProcessSentence` filters low-signal entries, enqueue + display guard maps (both keyed by `sentence.id`) block near-duplicates, and we skip items already in the queue, already pinned, or recently rendered.
...
4. **Resets & Telemetry Prep**
	- `handleTopicShift` cancels in-flight requests, clears pending queue items, and wipes guard maps while leaving existing chips/pins intact. `clearAll` layers on a prod reset for demo refreshes.
